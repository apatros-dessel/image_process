...
path2xls = r'd:\digital_earth\kanopus_new\vector\kanopus_report_v1.xls'
...
for rowkey in xls_dict:
    ...
    # print(path0)
    # path1 = path0.replace(r'F:\102_2020_116', r'd:\digital_earth\kanopus_new\krym')
    fullpath_list.append(path1)

scroll(fullpath_list, header='Raster scenes fullpaths:')
print('"{}"'.format(fullpath_list[0]))
proc = process().input(copy(fullpath_list))
print(len(proc))
# proc = process().input(fullpath_list[0:1])
# print(len(proc))
# raise Exception
...
for strip_id in raster_path_dict:
    ...
    for path2raster in raster_path_list:
        path2export = r'{}\{}\{}\DEFLATE_{}'.format(folder, sat, strip_id[-8:], os.path.split(path2raster)[1])
