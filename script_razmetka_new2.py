...
pin = [r'\\172.21.195.2\FTP-Share\ftp\landsat\done\lans8']                  # Путь к исходным файлам (растровым или растровым и векторным), можно указать список из нескольких директорий
vin = None     # Путь к векторному файлу масок (если None или '', то ведётся поиск векторных файлов в директории pin)
pout = r'e:\rks\razmetka\lsat8'                  # Путь для сохранения конечных файлов
imgid = 'IM4'                   # Индекс изображений (управляет числом каналов в конечном растре)
maskid = u'вода'                # Индекс масок (MWT, MFS и т.д.)
...
replace_vals = {7:100, 1:9, 2:9}         # Изменить значения в конечной маске в соответствии со словарём, если None, то замены не производится
