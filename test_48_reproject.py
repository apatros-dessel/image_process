...
def raster_match(path1, path2, method=gdal.GRA_NearestNeighbour):
...
for file in files:
    ...
    if re.search(align_renaming_template[0], n.lower()):
        ...
        for i, name in enumerate(names):
            ...
            if raster_match(align_file_new, file, method=gdal.GRA_NearestNeighbour)==3:
    ...
    for ref in references_in:
        match = raster_match(align_file, ref, method = gdal.GRA_NearestNeighbour)
        if match==3:
            if os.path.exists(pout):
                continue
