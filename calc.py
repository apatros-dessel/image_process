        ...
        try:
            ...
            if sign in ('==', '!='):
                if not isinstance(limits, tuple):
                    print('Wrong limit type for band {}: {}; a tuple is needed, trying to fix'.format(i+1, type(limits)))
                    try:
                        limits = tuple(limits)
                    except:
                        limits = (limits)
                ...
                if isinstance(limits, (tuple, list)):
                    print('Wrong limit type for band {}: {}; numeric is needed, trying to fix'.format(i+1, type(limits)))
                    limits = limits[0]
                for band in raster_data:
                    if sign == '>':
