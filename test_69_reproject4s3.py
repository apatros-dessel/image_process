...
folder_base = r'\\172.21.195.215\thematic\source\ntzomz'

folder_in_list = []

folder_base2 = r'\\172.21.195.215\thematic\source\ntzomz\__selected'
for name in os.listdir(folder_base2):
    folder_in = fullpath(folder_base2, name)
    if os.path.isdir(folder_in) and (not name.startswith('_')):
        folder_in_list.append(folder_in)

scroll(folder_in_list)
...
for folder_in in folder_in_list:
    ...
    matched, unmatched = GetUnmatchingScenes(source_scenes, s3_scenes, pms=pms)
