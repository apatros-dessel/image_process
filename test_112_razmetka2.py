...
output_folder = r'e:\rks\razmetka\set154__20211018__kanopus_ms_all__q30__12bit__edit002'
razmetka = Razmetka(mask_type = 'full', band_type = 'MS', vec_type = 'shp_hand', quicklook_size = 30)
for (category, cut) in categories:
    count = razmetka.EnterInput(corner, category, bands = None, use_empty = False, use_cut = cut, burn = None, filter_nodata = True, by_bands = 1, appendix_id = '_12bit', allow_corner_vector = True)
    print('Обнаружено %i сцен категории %s ; use_cut = %i' % (count, category, int(cut)))
    for bandid, bands in bands_params:
        check_count = razmetka.EnterCheck(corner, category, bands = None, use_cut = cut, by_bands = False, appendix_id = '_12bit')
    # print('Обнаружено %i контрольных сцен категории %s' % (check_count, category))
# count1 = razmetka.EnterInput(corner, 'strips', bands = None, use_empty = False, use_cut = 0, burn = None, filter_nodata = True, appendix_id = 'strips__12bit', allow_corner_vector = True)
# count2 = razmetka.EnterInput(corner, 'strips', bands = None, use_empty = False, use_cut = 1, burn = None, filter_nodata = True, appendix_id = 'strips__12bit', allow_corner_vector = True)
# print('Обнаружено %i сцен категории %s' % (count1 + count2, 'strips'))
# scroll(razmetka.input, counts=1)
# razmetka.MakeRazmetka(output_folder, del_codes = [232], cut_bits_limit = True, min_bits_limit = 16, overwrite = False, filter_mark = None, control_lim = 6,
...
for filter_mark in ['red', 'green', 'blue', 'nir']:
    i += 1
    output_folder = r'e:\rks\razmetka\set%i__20211012__kanopus_ms_all_bands_%s__q30__12bit' % (i, filter_mark)
