...
# Returns orthogonal line by three point coordinate values with definition of line length
def random_rectangle_line2(x1, y1, dx2, dy2, dx3, dy3, length = None):
    ...
    if length is None:
        ...
        len_max = math.sqrt(dx3**2+dy3**2)
        coef = length/max([length, len_max])
        # print(random1, random2, len_max, length, coef)

        x2 = x1 + dx2 * random1 + (1-coef) * dx3 * random2
        y2 = y1 + dy2 * random1 + (1-coef) * dy3 * random2
...
def RandomLinesRectangle(path_in, path_out,
    ...
    if lines_length is None:
        lines_length = math.sqrt(dx3**2+dy3**2)

    for i in range(lines_num):

        if random_length:
            length = lines_length * np.random.random()
        else:
            length = lines_length

        # new_line_geom = random_rectangle_line(x1, y1, dx2, dy2, dx3, dy3)
        new_line_geom = random_rectangle_line2(x1, y1, dx2, dy2, dx3, dy3, length=length)
